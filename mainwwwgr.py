...
mval.append (["VK.Добавления",df['adds_vk'],"lines"])
mval.append (["VK.Прослушивания",df['hears_vk'],"lines"])
mval.append (["VK.Эффективность",(df['adds_vk']) * 1000 / df['hears_vk'],"lines+markers"])
...
for i in range(len(mval)):
    if chk[i]==True:
        fig.add_trace(go.Scatter(x=created_time, y=mval[i][1], mode=mval[i][2], name=mval[0][0]))
