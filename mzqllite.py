...
class cDb(object):
    ...
    def graph_tracks(self, song, artist):
        ...
        z_CrestFactor = 100/df['eff_vk']
        created_time = df['mcurdate']

        fig = go.Figure()
        fig.add_trace(go.Scatter(x=created_time, y=x_CrestFactor, mode='lines', name='Добавления'))
        fig.add_trace(go.Scatter(x=created_time, y=y_CrestFactor, mode='lines', name='Прослушивания'))
        fig.add_trace(go.Scatter(x=created_time, y=z_CrestFactor, mode='lines+markers', name='Эффективность"'))
